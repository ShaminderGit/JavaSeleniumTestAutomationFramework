...
public class StandAloneTest {
        ...
        public static void main(String[] args) {
            ...
            options.addArguments(new String[]{"--proxy-server='direct://'"});
            ...
            inputEmail.sendKeys(new CharSequence[]{email});
            inputPassword.sendKeys(new CharSequence[]{password});
            loginButton.click();
            List<WebElement> navigationOptions = driver.findElements(By.cssSelector(".btn.btn-custom"));
            navigationOptions.forEach((option) -> System.out.println(option.getText()));
            ...
            WebElement shoe = (WebElement)products.stream().filter((product) -> product.findElement(By.cssSelector("h5 b")).getText().equalsIgnoreCase(productName)).findFirst().orElse(null);
            ...
            WebElement cartButton = (WebElement)navigationOptions.stream().filter((option) -> option.getText().contains("Cart")).findFirst().orElse(null);
            cartButton.click();
            List<WebElement> itemsInCart = driver.findElements(By.cssSelector(".cartWrap .items .infoWrap h3"));
            itemsInCart.forEach((option) -> System.out.println(option.getText()));
            ...
            action.sendKeys(selectCountry, new CharSequence[]{"India"}).build().perform();
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".ta-results")));
            List<WebElement> dropdownOptions = driver.findElements(By.cssSelector("button.ta-item span i"));
            dropdownOptions.forEach((option) -> System.out.println(option.getText()));
            ...
            WebElement signOutButton = (WebElement)navigationOptionstwo.stream().filter((option) -> option.getText().contains("Sign Out")).findFirst().orElse((null));
            signOutButton.click();
            driver.close();
            driver.quit();
        }
